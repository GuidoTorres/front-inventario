...
const RegistrarTrabajador = ({
  ...
  const getEquipos = async () => {
    const response = await fetch("http://localhost:3005/api/v1/equipos/select");
  ...
  const postTrabajador = async () => {
    if (editar) {
      const response = await fetch(
        `http://localhost:3005/api/v1/trabajadores/${editar.id}`,
        {
          method: "PUT",
          headers: {
      ...
      const response = await fetch(
        "http://localhost:3005/api/v1/trabajadores",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
